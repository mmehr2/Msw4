...
IMPLEMENT_DYNAMIC(AEmbeddedItem, COleServerItem)

extern CLIPFORMAT cfRTF;

AEmbeddedItem::AEmbeddedItem(AMswDoc* pContainerDoc, int nBeg, int nEnd)
...
AMswView* AEmbeddedItem::GetView() const
{
	...
	AMswView* pView = (AMswView*)pDoc->GetNextView(pos);
	ASSERT_VALID(pView);
	ASSERT(pView->IsKindOf(RUNTIME_CLASS(AMswView)));
	return pView;
}

void AEmbeddedItem::Serialize(CArchive& ar)
{
...
BOOL AEmbeddedItem::OnGetExtent(DVASPECT dwDrawAspect, CSize& rSize)
{
...
BOOL AEmbeddedItem::OnDraw(CDC* pDC, CSize& rSize)
{
	return OnDrawEx(pDC, rSize, TRUE);
}

BOOL AEmbeddedItem::OnDrawEx(CDC* pDC, CSize& rSize, BOOL bOutput)
{
	ADisplayIC dc;
	AMswView* pView = GetView();
	if (pView == NULL)
		return FALSE;
	ASSERT_VALID(pView);
